...
cfgs: Dict[str, List[Union[str, int]]] = {
    ...
    "VGG19v3": [16, 16, "M", 32, 32, "M", 64, 64, 64, 64, "M", 128, 128, 128, 128, "M", 128, 128, 128, 128, "M", 128, 256],
    ...
    "VGG19v4": [16, 16, "M", 32, 32, "M", 64, 64, 64, 64, "M", 128, 128, 128, 128, "M", 128, 128, 128, 128, "M", 256, 256],
    ...
    "VGG19v4sa": [16, 16, "M", 32, 32, "M", 64, 64, 64, 64, "M", 128, 128, 128, 128, "SAM", 128, 128, 128, 128, "SAM", 256, 256],
}


class VGG(nn.Module):
    def __init__(self, features: nn.ModuleList, classifier: nn.Module, expected_feature_shape: Tuple = (7, 7)) -> None:
        super().__init__()
...
class VGGKAN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun_first = partial(
            kan_conv3x3, spline_order=spline_order, grid_size=grid_size, base_activation=base_activation, grid_range=grid_range, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer
        )
        kan_fun = partial(mlp_kan, spline_order=spline_order, grid_size=grid_size, base_activation=base_activation, grid_range=grid_range, dropout=dropout_linear, l1_decay=l1_decay)
...
class FastVGGKAN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(
            fast_kan_conv3x3, grid_size=grid_size, base_activation=base_activation, grid_range=grid_range, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer
        )
        conv_fun_first = partial(fast_kan_conv3x3, grid_size=grid_size, base_activation=base_activation, grid_range=grid_range, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer)
        kan_fun = partial(mlp_fastkan, grid_size=grid_size, base_activation=base_activation, grid_range=grid_range, dropout=dropout_linear, l1_decay=l1_decay)
...
class VGGKALN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(kaln_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer)
...
class VGGKAGN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        norm_layer: nn.Module = nn.InstanceNorm2d,
        last_attention: bool = False,
        sa_inner_projection: int = None,
    ):
        conv_fun = partial(kagn_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer)
        conv_fun_first = partial(kagn_conv3x3, degree=degree, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer)
        kan_fun = partial(mlp_kagn, degree=degree, dropout=dropout_linear, l1_decay=l1_decay)
        kan_att_fun = partial(self_kagn_conv3x3, degree=degree, inner_projection=sa_inner_projection, dropout=dropout, groups=groups, affine=affine, norm_layer=norm_layer)
...
class VGGKAGN_BN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(bottleneck_kagn_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer)
        conv_fun_first = partial(bottleneck_kagn_conv3x3, degree=degree, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer)
        kan_fun = partial(mlp_kagn, degree=degree, dropout=dropout_linear, l1_decay=l1_decay)
        kan_att_fun = partial(self_bottleneck_kagn_conv3x3, degree=degree, inner_projection=sa_inner_projection, dropout=dropout, groups=groups, affine=affine, norm_layer=norm_layer)
...
class MoE_VGGKAGN_BN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(
            moe_bottleneck_kagn_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer, num_experts=num_experts, k=k, noisy_gating=noisy_gating
        )
        conv_fun_first = partial(bottleneck_kagn_conv3x3, degree=degree, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer)
        kan_fun = partial(mlp_kagn, degree=degree, dropout=dropout_linear, l1_decay=l1_decay)
        kan_att_fun = partial(self_bottleneck_kagn_conv3x3, degree=degree, inner_projection=sa_inner_projection, dropout=dropout, groups=groups, affine=affine, norm_layer=norm_layer)
...
class VGGKACN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(kacn_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, affine=affine, norm_layer=norm_layer)
        conv_fun_first = partial(kacn_conv3x3, degree=degree, l1_decay=l1_decay, affine=affine, norm_layer=norm_layer)
        kan_fun = partial(mlp_kacn, degree=degree, dropout=dropout_linear, l1_decay=l1_decay)
...
class MoEVGGKAGN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(moe_kagn_conv3x3, degree=degree, dropout=dropout, l1_decay=l1_decay, groups=groups, num_experts=num_experts, noisy_gating=noisy_gating, k=k, affine=affine)
...
class WavVVGKAN(VGG, PyTorchModelHubMixin):
    def __init__(
        ...
        conv_fun = partial(wav_kan_conv3x3, wavelet_type=wavelet_type, wav_version=wav_version, dropout=dropout, l1_decay=l1_decay, groups=groups, norm_layer=norm_layer, affine=affine)
        conv_fun_first = partial(wav_kan_conv3x3, wavelet_type=wavelet_type, wav_version=wav_version, l1_decay=l1_decay, norm_layer=norm_layer, affine=affine)
        kan_fun = partial(mlp_wav_kan, wavelet_type=wavelet_type, dropout=dropout_linear, l1_decay=l1_decay)
